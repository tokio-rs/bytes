...
use bytes::{BufMut, IntoBuf, LocalBytes, LocalBytesMut};
...
#[test]
fn from_slice() {
    let a = LocalBytes::from(&b"abcdefgh"[..]);
    ...
    let a = LocalBytesMut::from(&b"abcdefgh"[..]);
    assert_eq!(a, b"abcdefgh"[..]);
    assert_eq!(a, &b"abcdefgh"[..]);
    assert_eq!(a, Vec::from(&b"abcdefgh"[..]));
    assert_eq!(b"abcdefgh"[..], a);
...
#[test]
fn fmt() {
    let a = format!("{:?}", LocalBytes::from(&b"abcdefg"[..]));
    let b = "b\"abcdefg\"";

    assert_eq!(a, b);

    let a = format!("{:?}", LocalBytesMut::from(&b"abcdefg"[..]));
    assert_eq!(a, b);
}
...
fn fmt_write() {
    ...
    let mut a = LocalBytesMut::with_capacity(64);
    write!(a, "{}", &s[..64]).unwrap();
    ...
    let mut c = LocalBytesMut::with_capacity(64);
...
#[test]
fn len() {
    let a = LocalBytes::from(&b"abcdefg"[..]);
    assert_eq!(a.len(), 7);

    let a = LocalBytesMut::from(&b"abcdefg"[..]);
    assert_eq!(a.len(), 7);

    let a = LocalBytes::from(&b""[..]);
    assert!(a.is_empty());

    let a = LocalBytesMut::from(&b""[..]);
    assert!(a.is_empty());
}
...
#[test]
fn split_off() {
    let mut hello = LocalBytes::from(&b"helloworld"[..]);
    let world = hello.split_off(5);

    assert_eq!(hello, &b"hello"[..]);
    assert_eq!(world, &b"world"[..]);

    let mut hello = LocalBytesMut::from(&b"helloworld"[..]);
    let world = hello.split_off(5);
...
fn split_off_uninitialized() {
    let mut bytes = LocalBytesMut::with_capacity(1024);
...
fn split_off_to_loop() {
    ...
    for i in 0..(s.len() + 1) {
        {
            let mut bytes = LocalBytes::from(&s[..]);
        ...
        {
            let mut bytes = LocalBytesMut::from(&s[..]);
            let off = bytes.split_off(i);
            assert_eq!(i, bytes.len());
            let mut sum = Vec::new();
            sum.extend(&bytes);
...
fn split_to_1() {
    // Inline
    let mut a = LocalBytes::from(SHORT);
    let b = a.split_to(4);
    ...
    // Allocated
    let mut a = LocalBytes::from(LONG);
    let b = a.split_to(4);
...
fn split_off_to_at_gt_len() {
    fn make_bytes() -> LocalBytes {
        let mut bytes = LocalBytesMut::with_capacity(100);
...
fn fns_defined_for_bytes_mut() {
    let mut bytes = LocalBytesMut::from(&b"hello world"[..]);
...
fn reserve_convert() {
    // Inline -> Vec
    let mut bytes = LocalBytesMut::with_capacity(8);
    bytes.put("hello");
    bytes.reserve(40);
    ...
    // Inline -> Inline
    let mut bytes = LocalBytesMut::with_capacity(inline_cap());
    ...
    // Vec -> Vec
    let mut bytes = LocalBytesMut::from(LONG);
    bytes.reserve(64);
...
fn reserve_max_original_capacity_value() {
    ...
    let mut bytes = LocalBytesMut::with_capacity(SIZE);
...
// Without either looking at the internals of the LocalBytesMut or doing weird stuff
...
fn reserve_in_arc_unique_does_not_overallocate() {
    let mut bytes = LocalBytesMut::with_capacity(1000);
    bytes.take();
...
#[test]
fn extend_mut() {
    let mut bytes = LocalBytesMut::with_capacity(0);
    bytes.extend(LONG);
...
#[test]
fn from_static() {
    let mut a = LocalBytes::from_static(b"ab");
    let b = a.split_off(1);
...
#[test]
fn advance_static() {
    let mut a = LocalBytes::from_static(b"hello world");
    a.advance(6);
    assert_eq!(a, &b"world"[..]);
}
...
#[test]
fn advance_vec() {
    let mut a = LocalBytesMut::from(b"hello world boooo yah world zomg wat wat".to_vec());
    a.advance(16);
...
#[should_panic]
fn advance_past_len() {
    let mut a = LocalBytesMut::from(b"hello world".to_vec());
    a.advance(20);
}

#[test]
fn partial_eq_bytesmut() {
    let bytes = LocalBytes::from(&b"The quick red fox"[..]);
    let bytesmut = LocalBytesMut::from(&b"The quick red fox"[..]);
    assert!(bytes == bytesmut);
    assert!(bytesmut == bytes);
    let bytes2 = LocalBytes::from(&b"Jumped over the lazy brown dog"[..]);
...
fn bytes_unsplit_empty_self() {
    // empty self
    let mut buf = LocalBytes::new();

    let mut other = LocalBytes::with_capacity(64);
...
fn bytes_unsplit_inline_arc() {
    let mut buf = LocalBytes::with_capacity(8); //inline
    buf.extend_from_slice(b"aaaabbbb");

    let mut buf2 = LocalBytes::with_capacity(64);
...
fn bytes_unsplit_arc_inline() {
    ...
    let mut buf2 = LocalBytes::with_capacity(8); //inline
...
fn bytes_unsplit_both_inline() {
    let mut buf = LocalBytes::with_capacity(16); //inline
...
fn bytes_mut_unsplit_empty_self() {
    ...
    let mut other = LocalBytesMut::with_capacity(64);
    other.extend_from_slice(b"aaabbbcccddd");
...
fn bytes_mut_unsplit_inline_arc() {
    let mut buf = LocalBytesMut::with_capacity(8); //inline
    buf.extend_from_slice(b"aaaabbbb");

    let mut buf2 = LocalBytesMut::with_capacity(64);
    buf2.extend_from_slice(b"ccccddddeeee");
...
fn bytes_mut_unsplit_arc_inline() {
    ...
    let mut buf2 = LocalBytesMut::with_capacity(8); //inline
    buf2.extend_from_slice(b"ccccdddd");
...
fn bytes_mut_unsplit_both_inline() {
    let mut buf = LocalBytesMut::with_capacity(16); //inline
    buf.extend_from_slice(b"aaaabbbbccccdddd");
...
fn from_iter_no_size_hint() {
    ...
    let actual: LocalBytes = iter::repeat(b'x')
        .scan(100, |cnt, item| {
            if *cnt >= 1 {
                *cnt -= 1;
                expect.push(item);
